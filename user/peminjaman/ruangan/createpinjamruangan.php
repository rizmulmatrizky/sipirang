								...
								<div class="form-group">
									...
									<select class="form-control" id="id_ruangan" onchange="change(this.value)" name="id_ruangan" required="">
										...
										while ($row = mysqli_fetch_array($query)) {
											...
											$deskripsi .= "deskripsi['" . $row['id_ruangan'] . "'] = {deskripsi:'" . addslashes($row['deskripsi']) . "'};\n";
											$nama_ruangan .= "nama_ruangan['" . $row['id_ruangan'] . "'] = {nama_ruangan:'" . addslashes($row['nama_ruangan']) . "'};\n";
										}
										?>
									</select>
								</div>
						...
						<form method="POST" action="" enctype="multipart/form-data">
							<div class="card-body">
								<div class="form-group">
									...
									<input type="email" name="email" placeholder="Email Pengirim ..." class="form-control" required="">
								</div>
								<div class="form-group">
									<label>Password Pengirim</label>
									<input type="password" name="password" placeholder="Password Pengirim ..." class="form-control" required="">
								</div>
...
if (isset($_POST['simpan'])) {
	...
	$selSto = mysqli_query($conn, "SELECT * FROM ruangan WHERE id_ruangan='$id_ruangan'");
	$sto    = mysqli_fetch_array($selSto);
	$stok    = $sto['status'];
	//menghitung sisa stok
	$sisa    = 'dipinjam';

	mysqli_query($conn, "INSERT into pemakaian values ('','$id_ruangan', '$user_id', '$tanggal_mulai', '$tanggal_selesai', '$status')");
	mysqli_query($conn, "UPDATE ruangan SET status='$sisa' WHERE id_ruangan='$id_ruangan'");
}
?>